...
import { Glicko, type Match, type Player } from "glicko-ts";
...
export function calculateGlicko1Rating({
	...
	const daysInactiveWhite = Math.floor(
		(Date.now() - new Date(whiteUserLastPlayedMatch).getTime()) / msPerDay,
	);
	const daysInactiveBlack = Math.floor(
		(Date.now() - new Date(blackUserLastPlayedMatch).getTime()) / msPerDay,
	);
